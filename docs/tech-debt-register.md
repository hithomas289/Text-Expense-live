...
- **Deploy note:** the most recent Railway deploy attempt (commit "d" by Divya0600) **FAILED**; production is currently served by an earlier successful deploy. `planType` enum has no `premium` (the ops-runbook monetization query references `'premium'` — harmless at 0 paid, but would error in an enum-cast context).
